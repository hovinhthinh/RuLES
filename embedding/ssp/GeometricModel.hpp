...
class TransE : public Model {
 ...
 public:
  TransE(const Dataset& dataset, const TaskType& task_type,
         const string& logging_base_path, int dim, double alpha,
    ...
    for_each(embedding_entity.begin(), embedding_entity.end(), [=](vec& elem) {
      elem = (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim);
    });

    embedding_relation.resize(count_relation());
    for_each(
        embedding_relation.begin(), embedding_relation.end(),
        [=](vec& elem) { elem = (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim); });
  }

  TransE(const Dataset& dataset, const string& file_zero_shot,
         const TaskType& task_type, const string& logging_base_path, int dim,
         double alpha, double training_threshold)
  ...
  virtual void draw(const string& filename, const int radius,
                    const int id_relation) const {
    mat record(radius * 6.0 + 10, radius * 6.0 + 10);
    record.fill(255);
    for (auto i = data_model.data_train.begin();
         i != data_model.data_train.end(); ++i) {
      if (i->second == id_relation) {
        record(radius * (3.0 + embedding_entity[i->first.second][0] -
                         embedding_entity[i->first.first][0]),
               radius * (3.0 + embedding_entity[i->first.second][1] -
                         embedding_entity[i->first.first][1])) = 0;
        record(radius * (3.0 + embedding_entity[i->first.second][0] -
                         embedding_entity[i->first.first][0]) +
                   1,
               radius * (3.0 + embedding_entity[i->first.second][1] -
                         embedding_entity[i->first.first][1]) +
                   1) = 0;
        record(radius * (3.0 + embedding_entity[i->first.second][0] -
                         embedding_entity[i->first.first][0]) +
                   1,
    ...
    record.save(filename + replace_all(relation_name, "/", "_") + ".ppm",
                pgm_binary);
  }

  virtual void draw(const string& filename, const int radius, int id_head,
                    int id_relation) {
    mat record(radius * 6.0 + 4, radius * 6.0 + 4);
    record.fill(255);
    for (auto i = data_model.rel_tails.at(id_relation).at(id_head).begin();
         i != data_model.rel_tails.at(id_relation).at(id_head).end(); ++i) {
      if (rand() % 100 < 80) continue;

      record(radius *
                 (3.0 + embedding_entity[*i][0] - embedding_entity[id_head][0] -
                  embedding_relation[id_relation][0]),
             radius *
                 (3.0 + embedding_entity[*i][1] - embedding_entity[id_head][1] -
                  embedding_relation[id_relation][1])) = 0;
      record(radius * (3.0 + embedding_entity[*i][0] -
                       embedding_entity[id_head][0] -
                       embedding_relation[id_relation][0]) +
                 1,
             radius * (3.0 + embedding_entity[*i][1] -
                       embedding_entity[id_head][1] -
                       embedding_relation[id_relation][1]) +
                 1) = 0;
      record(radius * (3.0 + embedding_entity[*i][0] -
                       embedding_entity[id_head][0] -
                       embedding_relation[id_relation][0]) +
    ...
    for (auto j = 0; j < 40; ++j) {
      ...
      record(radius *
             ...
             radius *
                 (3.0 + embedding_entity[i][1] - embedding_entity[id_head][1] -
                  embedding_relation[id_relation][1])) = 1;
      record(radius * (3.0 + embedding_entity[i][0] -
                       embedding_entity[id_head][0] -
                       embedding_relation[id_relation][0]) +
                 1,
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    if (norm_L2(relation) > 1.0) relation = normalise(relation);
  ...
  virtual void relation_reg(int i, int j, double factor) {
    ...
    embedding_relation[i] -=
        alpha * factor *
        sign(as_scalar(embedding_relation[i].t() * embedding_relation[j])) *
        embedding_relation[j];
    embedding_relation[j] -=
        alpha * factor *
        sign(as_scalar(embedding_relation[i].t() * embedding_relation[j])) *
        embedding_relation[i];
  ...
  virtual void entity_reg(int i, int j, double factor) {
    ...
    embedding_entity[i] -=
        alpha * factor *
        sign(as_scalar(embedding_entity[i].t() * embedding_entity[j])) *
        embedding_entity[j];
    embedding_entity[j] -=
        alpha * factor *
        sign(as_scalar(embedding_entity[i].t() * embedding_entity[j])) *
...
class TransE_ESS : public TransE {
 protected:
  const double ESS_factor;

 public:
  TransE_ESS(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold, double ESS_factor)
      : TransE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold),
...
class TransH : public TransE {
 protected:
  vector<vec> embedding_weights;

 public:
  TransH(const Dataset& dataset, const TaskType& task_type,
         const string& logging_base_path, int dim, double alpha,
         double training_threshold)
      : TransE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold) {
    ...
    for_each(
        embedding_weights.begin(), embedding_weights.end(),
        [=](vec& elem) { elem = (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim); });
  }

  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    vec error = embedding_entity[triplet.first.first] -
                as_scalar(embedding_weights[triplet.second].t() *
                          embedding_entity[triplet.first.first]) *
                    embedding_weights[triplet.second] +
                embedding_relation[triplet.second] -
                embedding_entity[triplet.first.second] +
                as_scalar(embedding_weights[triplet.second].t() *
                          embedding_entity[triplet.first.second]) *
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    vec factor_true =
        sign(head - as_scalar(weight.t() * head) * weight + relation - tail +
             as_scalar(weight.t() * tail) * weight);
    vec factor_false =
        sign(head_f - as_scalar(weight.t() * head_f) * weight + relation -
             tail_f + as_scalar(weight.t() * tail_f) * weight);
    ...
    weight += 2 * alpha * (factor_true * as_scalar(head.t() * weight) -
                           factor_false * as_scalar(tail.t() * weight));
    ...
    if (norm_L2(weight) > 1.0) weight = normalise(weight);
  }
};
class TransA : public TransE {
 protected:
  vector<mat> mat_r;

 public:
  TransA(const Dataset& dataset, const TaskType& task_type,
         const string& logging_base_path, int dim, double alpha,
         double training_threshold)
      : TransE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold) {
 ...
 public:
  ...
  virtual double training_prob_triplets(
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    if (training_prob_triplets(triplet) - training_prob_triplets(triplet_f) >
        training_threshold)
      return;
    ...
    if (last_time || task_type == TripletClassification) {
      ...
      for (auto i = data_model.data_train.begin();
        ...
        mat_r[triplet.second] -=
            ...
            abs(head_f + relation_f - tail_f) *
                abs(head_f + relation_f - tail_f).t();
      }
      for_each(mat_r.begin(), mat_r.end(),
               [=](mat& elem) { elem = normalise(elem); });
    }
  }
...
class TransA_PSD : public TransE {
 protected:
  vector<mat> mat_r;

 public:
  TransA_PSD(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold)
      : TransE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold) {
 ...
 public:
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    return -as_scalar((error).t() * mat_r[triplet.second] *
                      mat_r[triplet.second].t() * (error));
  }
    ...
    if (last_time || task_type == TripletClassification) {
      ...
      // for(auto i=0; i<10; ++i)
      {
        for (auto i = data_model.data_train.begin();
          ...
          -0.001 * (head_f + relation_f - tail_f) *
              (head_f + relation_f - tail_f).t();
        }
        for_each(mat_r.begin(), mat_r.end(),
                 [=](mat& elem) { elem = elem.i(); });
      }
    }
  }
...
class TransA_PSD_ESS : public TransE {
 protected:
  vector<mat> mat_r;
  const double ESS_factor;

 public:
  TransA_PSD_ESS(const Dataset& dataset, const TaskType& task_type,
                 const string& logging_base_path, int dim, double alpha,
                 double training_threshold, double ESS_factor)
      : TransE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold),
...
class TransA_ESS : public TransA {
 protected:
  const double ESS_factor;

 public:
  TransA_ESS(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold, double ESS_factor)
      : TransA(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold),
        ESS_factor(ESS_factor) {
...
class TransM : public Model {
 ...
 public:
  TransM(const Dataset& dataset, const TaskType& task_type,
         const string& logging_base_path, int dim, double alpha,
         double training_threshold, int n_cluster, double sparse_factor,
         bool sot = true, bool be_weight_normalized = false)
      : Model(dataset, task_type, logging_base_path),
        dim(dim),
        alpha(alpha),
        training_threshold(training_threshold),
        ...
        single_or_total(sot),
        be_weight_normalized(be_weight_normalized) {
 ...
 public:
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    if (single_or_total == false) return training_prob_triplets(triplet);

    double mixed_prob = 1e-8;
    for (int c = 0; c < n_cluster; ++c) {
      vec error_c = embedding_entity[triplet.first.first] +
                    embedding_clusters[triplet.second][c] -
                    embedding_entity[triplet.first.second];
  ...
  virtual double training_prob_triplets(
    ...
    for (int c = 0; c < n_cluster; ++c) {
      vec error_c = embedding_entity[triplet.first.first] +
                    ...
                    embedding_entity[triplet.first.second];
      mixed_prob +=
          fabs(weights_clusters[triplet.second][c]) * exp(-sum(abs(error_c)));
    }
  ...
  virtual void train_cluster_once(const pair<pair<int, int>, int>& triplet,
                                  const pair<pair<int, int>, int>& triplet_f,
                                  int cluster, double prob_true,
                                  double prob_false, double factor) {
    vec& head = embedding_entity[triplet.first.first];
    vec& tail = embedding_entity[triplet.first.second];
    ...
    weights_clusters[triplet.second][cluster] +=
        alpha / prob_true * prob_local_true *
        sign(weights_clusters[triplet.second][cluster]);
    weights_clusters[triplet_f.second][cluster] -=
        alpha / prob_false * prob_local_false *
        sign(weights_clusters[triplet_f.second][cluster]);

    weights_clusters[triplet.second][cluster] -=
        alpha * sign(weights_clusters[triplet.second][cluster]);

    head -= alpha * sign(head + relation - tail) * prob_local_true / prob_true *
            fabs(weights_clusters[triplet.second][cluster]);
    tail += alpha * sign(head + relation - tail) * prob_local_true / prob_true *
            fabs(weights_clusters[triplet.second][cluster]);
    relation -= alpha * sign(head + relation - tail) * prob_local_true /
                prob_true * fabs(weights_clusters[triplet.second][cluster]);
    head_f += alpha * sign(head_f + relation_f - tail_f) * prob_local_false /
              prob_false * fabs(weights_clusters[triplet.second][cluster]);
    tail_f -= alpha * sign(head_f + relation_f - tail_f) * prob_local_false /
              prob_false * fabs(weights_clusters[triplet.second][cluster]);
    relation_f += alpha * sign(head_f + relation_f - tail_f) *
                  prob_local_false / prob_false *
                  fabs(weights_clusters[triplet.second][cluster]);

    if (norm_L2(relation) > 1.0) relation = normalise(relation);

    if (norm_L2(relation_f) > 1.0) relation_f = normalise(relation_f);
  }
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    if (prob_true / prob_false > training_threshold) return;
    ...
    if (be_weight_normalized)
      weights_clusters[triplet.second] =
          normalise(weights_clusters[triplet.second]);
  }
 ...
 public:
  virtual void report(const string& filename) const {
    if (task_type == TransM_ReportClusterNumber) {
      ...
      for (auto i = weights_clusters.begin(); i != weights_clusters.end();
           ++i) {
        int n = 0;
        for_each(i->begin(), i->end(), [&](double w) {
          if (fabs(w) > 0.005) ++n;
        });
        cout << data_model.relation_id_to_name[i - weights_clusters.begin()]
             << ":" << n << endl;
        ++count_cluster[n];
        total_number += n;
      }
      copy(count_cluster.begin(), count_cluster.end(),
...
class TransE_SW : public TransE {
 protected:
  const double wake_factor;

 public:
  TransE_SW(const Dataset& dataset, const TaskType& task_type,
            const string& logging_base_path, int dim, double alpha,
            double training_threshold, double wake_factor)
  ...
  virtual void train_triplet(
      const pair<pair<int, int>, int>& triplet) override {
    vec& head = embedding_entity[triplet.first.first];
    vec& tail = embedding_entity[triplet.first.second];
    vec& relation = embedding_relation[triplet.second];
    ...
    for_each(grad.begin(), grad.end(), [&](double& elem) {
      if (abs(elem) < wake_factor) elem = 0;
    });
    grad = sign(grad);
    ...
    for_each(grad_f.begin(), grad_f.end(), [&](double& elem) {
      if (abs(elem) < wake_factor) elem = 0;
    });
    grad_f = sign(grad_f);
...
class TransG : public Model {
 ...
 public:
  TransG(const Dataset& dataset, const TaskType& task_type,
         const string& logging_base_path, int dim, double alpha,
         double training_threshold, int n_cluster, double CRP_factor,
         int step_before = 10, bool sot = false,
    ...
    this->CRP_factor =
        CRP_factor / data_model.data_train.size() * count_relation();
  }
 ...
 public:
  ...
  virtual void train_cluster_once(const pair<pair<int, int>, int>& triplet,
    ...
    weights_clusters[triplet.second][cluster] +=
        factor / prob_true * prob_local_true *
        sign(weights_clusters[triplet.second][cluster]);
    weights_clusters[triplet_f.second][cluster] -=
        factor / prob_false * prob_local_false *
        sign(weights_clusters[triplet_f.second][cluster]);

    head -= factor * sign(head + relation - tail) * prob_local_true /
            prob_true * fabs(weights_clusters[triplet.second][cluster]);
    tail += factor * sign(head + relation - tail) * prob_local_true /
            prob_true * fabs(weights_clusters[triplet.second][cluster]);
    relation -= factor * sign(head + relation - tail) * prob_local_true /
                prob_true * fabs(weights_clusters[triplet.second][cluster]);
    head_f += factor * sign(head_f + relation_f - tail_f) * prob_local_false /
              prob_false * fabs(weights_clusters[triplet.second][cluster]);
    tail_f -= factor * sign(head_f + relation_f - tail_f) * prob_local_false /
              prob_false * fabs(weights_clusters[triplet.second][cluster]);
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    if (prob_true / prob_false > exp(training_threshold)) return;
    ...
    if (randu() < prob_new_component / (prob_new_component + prob_true) &&
        size_clusters[triplet.second] < 20 && epos >= step_before) {
#pragma omp critical
      {
        // cout<<"A";
        weights_clusters[triplet.second][size_clusters[triplet.second]] =
            CRP_factor;
        embedding_clusters[triplet.second][size_clusters[triplet.second]] =
            (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim);  // tail - head;
...
class TransG_Hiracherical : public Model {
 protected:
  vector<vec> embedding_entity;
  vector<vector<vec>> embedding_clusters;
  vector<vec> weights_clusters;
 ...
 public:
  TransG_Hiracherical(const Dataset& dataset, const TaskType& task_type,
                      const string& logging_base_path, int dim, double alpha,
                      double training_threshold, int n_cluster,
                      double CRP_factor, int step_before = 10,
                      double variance_bound = 0.01, bool sot = false,
                      bool be_weight_normalized = true)
      : Model(dataset, task_type, logging_base_path),
        dim(dim),
        alpha(alpha),
 ...
 public:
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    double total_variance =
        variance[triplet.first.first] * variance[triplet.first.first] +
    ...
    for (int c = 0; c < size_clusters[triplet.second]; ++c) {
      vec error_c = (embedding_entity[triplet.first.first] +
                     ...
                     embedding_entity[triplet.first.second]);
      mixed_prob =
          max(mixed_prob, fabs(weights_clusters[triplet.second][c]) *
  ...
  virtual void train_cluster_once(const pair<pair<int, int>, int>& triplet,
    ...
    double total_variance =
        variance[triplet.first.first] * variance[triplet.first.first] +
        variance[triplet.first.second] * variance[triplet.first.second] + 1;
    double total_variance_f =
        variance[triplet_f.first.first] * variance[triplet_f.first.first] +
        variance[triplet_f.first.second] * variance[triplet_f.first.second] + 1;
    double prob_local_true =
        exp(-sum(abs(head + relation - tail)) / total_variance);
    double prob_local_false =
        exp(-sum(abs(head_f + relation_f - tail_f)) / total_variance_f);

    const double thres = -variance_bound;

    variance[triplet.first.first] +=
        alpha * 2 * fabs(weights_clusters[triplet.second][cluster]) *
        prob_local_true / prob_true * sum(abs(head + relation - tail)) /
        total_variance / total_variance * variance[triplet.first.first];
    variance[triplet.first.first] =
        max(-thres, min(thres, variance[triplet.first.first]));

    variance[triplet.first.second] +=
        alpha * 2 * fabs(weights_clusters[triplet.second][cluster]) *
        prob_local_true / prob_true * sum(abs(head + relation - tail)) /
        total_variance / total_variance * variance[triplet.first.second];
    variance[triplet.first.second] =
        max(thres, min(thres, variance[triplet.first.second]));

    variance[triplet_f.first.first] -=
        alpha * 2 * fabs(weights_clusters[triplet_f.second][cluster]) *
    ...
    variance[triplet_f.first.first] =
        max(thres, min(thres, variance[triplet_f.first.first]));

    variance[triplet_f.first.second] -=
        alpha * 2 * fabs(weights_clusters[triplet_f.second][cluster]) *
        prob_local_false / prob_false * sum(abs(head_f + relation_f - tail_f)) /
        total_variance_f / total_variance_f * variance[triplet_f.first.second];
    variance[triplet_f.first.second] =
        max(thres, min(thres, variance[triplet_f.first.second]));
    ...
    head -= alpha * sign(head + relation - tail) * prob_local_true / prob_true *
            fabs(weights_clusters[triplet.second][cluster]) / total_variance;
    tail += alpha * sign(head + relation - tail) * prob_local_true / prob_true *
            fabs(weights_clusters[triplet.second][cluster]) / total_variance;
    relation -= alpha * sign(head + relation - tail) * prob_local_true /
                prob_true * fabs(weights_clusters[triplet.second][cluster]) /
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double prob_new_component =
        CRP_factor * exp(-sum(abs(head - tail))) /
...
#pragma omp critical
      {
        ...
        weights_clusters[triplet.second][size_clusters[triplet.second]] =
            CRP_factor;
        embedding_clusters[triplet.second][size_clusters[triplet.second]] =
            tail - head;  //(2*randu(dim,1)-1)*sqrt(6.0/dim);
        ++size_clusters[triplet.second];
      }
    }
