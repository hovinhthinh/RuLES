...
class SemanticModel : public TransE {
 protected:
 ...
 public:
  SemanticModel(const Dataset& dataset, const TaskType& task_type,
                const string& logging_base_path, const string& semantic_file,
                int dim, double alpha, double training_threshold,
                double balance = 0.1)
  ...
  SemanticModel(const Dataset& dataset, const TaskType& task_type,
                ...
                double training_threshold, double balance = 0.1)
      : TransE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold),
        balance(balance) {
    logging.record() << "\t[Name]\tSemanticModel.TransE";
  ...
  SemanticModel(const Dataset& dataset, const string& file_zeroshot,
                const TaskType& task_type, const string& logging_base_path,
                const string& semantic_file, int dim, double alpha,
                double training_threshold, double balance = 0.1)
      : TransE(dataset, file_zeroshot, task_type, logging_base_path, dim, alpha,
               training_threshold),
        balance(balance) {
    logging.record() << "\t[Name]\tSemanticModel.TransE";
    logging.record() << "\t[Dimension]\t" << dim;
 ...
 public:
  virtual const vec semantic_composition(
      const pair<pair<int, int>, int>& triplet) const {
    return (v_semantics[triplet.first.first] +
            v_semantics[triplet.first.second]) /
           max(1e-5, sum(abs(v_semantics[triplet.first.first] +
                             v_semantics[triplet.first.second])));
    return normalise(v_semantics[triplet.first.first] +
                     v_semantics[triplet.first.second]);
  }
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    return -balance *
               sum(abs(error - as_scalar(semantic.t() * error) * semantic)) -
           sum(abs(error));
  }
...
class SemanticModel_Joint : public SemanticModel {
 ...
 public:
  SemanticModel_Joint(const Dataset& dataset, const TaskType& task_type,
                      const string& logging_base_path,
                      const string& semantic_file_raw, int dim, double alpha,
                      double training_threshold, double balance, double factor)
      : SemanticModel(dataset, task_type, logging_base_path, dim, alpha,
                      training_threshold, balance),
        factor(factor) {
  ...
  SemanticModel_Joint(const Dataset& dataset, const TaskType& task_type,
                      ...
                      const string& semantic_file_raw, int dim, double alpha,
                      double training_threshold, double balance, double factor)
      : SemanticModel(dataset, task_type, logging_base_path, semantic_file, dim,
                      alpha, training_threshold, balance),
        factor(factor) {
    logging.record() << "\t[Name]\tSemanticModel.Joint";
    logging.record() << "\t[Factor]\t" << factor;
  ...
  SemanticModel_Joint(const Dataset& dataset, const string& file_zeroshot,
                      const TaskType& task_type,
                      const string& logging_base_path,
                      const string& semantic_file,
                      const string& semantic_file_raw, int dim, double alpha,
                      double training_threshold, double balance, double factor)
      : SemanticModel(dataset, file_zeroshot, task_type, logging_base_path,
                      semantic_file, dim, alpha, training_threshold, balance),
        factor(factor) {
    logging.record() << "\t[Name]\tSemanticModel.Joint";
    logging.record() << "\t[Factor]\t" << factor;
  ...
  SemanticModel_Joint(const Dataset& dataset, const string& file_zeroshot,
                      ...
                      const string& semantic_file_raw, int dim, double alpha,
                      double training_threshold, double balance, double factor)
      : SemanticModel(dataset, file_zeroshot, task_type, logging_base_path, dim,
                      alpha, training_threshold, balance),
        factor(factor) {
    logging.record() << "\t[Name]\tSemanticModel.Joint";
    logging.record() << "\t[Factor]\t" << factor;
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    head_sem += alpha * balance * factor * projection *
                (sign(error - projection * semantic) -
                 as_scalar(semantic.t() * sign(error - projection * semantic)) *
    ...
    relation_f += alpha * balance * grad_f + alpha * sign(error_f);
    head_sem_f -=
        alpha * balance * factor * projection_f *
        (sign(error_f - projection_f * semantic_f) -
         as_scalar(semantic_f.t() * sign(error_f - projection_f * semantic_f)) *
...
class SemanticModel_ZeroShot : public SemanticModel_Joint {
 protected:
  mat mat_transfer;

 public:
  SemanticModel_ZeroShot(const Dataset& dataset, const string& file_zeroshot,
                         const TaskType& task_type,
                         const string& logging_base_path,
                         const string& semantic_file,
                         const string& semantic_file_raw, int dim, double alpha,
                         double training_threshold, double balance,
                         double factor)
      : SemanticModel_Joint(dataset, file_zeroshot, task_type,
                            logging_base_path, semantic_file, semantic_file_raw,
                            dim, alpha, training_threshold, balance, factor),
