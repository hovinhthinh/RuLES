...
class Model {
 ...
 public:
  Model(const Dataset& dataset, const TaskType& task_type,
        const string& logging_base_path)
      : data_model(*(new DataModel(dataset))),
        task_type(task_type),
  ...
  Model(const Dataset& dataset, const string& file_zero_shot,
        const TaskType& task_type, const string& logging_base_path)
      : data_model(*(new DataModel(dataset))),
        task_type(task_type),
        logging(*(new ModelLogging(logging_base_path))),
        be_deleted_data_model(true) {
  ...
  Model(const DataModel* data_model, const TaskType& task_type,
        ModelLogging* logging)
...
#pragma omp parallel for
  ...
  void run(int total_epos) {
    ...
    while (total_epos-- > 0) {
      ...
      if (task_type == TripletClassification) test_triplet_classification();
    }
 ...
 public:
  ...
  void test(int hit_rank = 10) {
    ...
    if (task_type == LinkPredictionHeadTail)
      test_link_prediction_head_tail(hit_rank);
    else if (task_type == LinkPredictionHead ||
             task_type == LinkPredictionTail ||
             task_type == LinkPredictionRelation)
      test_link_prediction(hit_rank);
    else if (task_type == LinkPredictionHeadZeroShot ||
             task_type == LinkPredictionTailZeroShot ||
             task_type == LinkPredictionRelationZeroShot)
 ...
 public:
  void test_triplet_classification() {
    ...
    for (auto r = 0; r < data_model.set_relation.size(); ++r) {
      ...
      for (auto i = data_model.data_dev_true.begin();
           i != data_model.data_dev_true.end(); ++i) {
        if (i->second != r) continue;

        threshold_dev.push_back(make_pair(prob_triplets(*i), true));
      }
      for (auto i = data_model.data_dev_false.begin();
           i != data_model.data_dev_false.end(); ++i) {
        if (i->second != r) continue;
      ...
      for (auto i = data_model.data_test_true.begin();
           i != data_model.data_test_true.end(); ++i) {
        if (i->second != r) continue;

        ++lreal_total;
        if (prob_triplets(*i) > threshold) ++real_hit, ++lreal_hit;
      }

      for (auto i = data_model.data_test_false.begin();
           i != data_model.data_test_false.end(); ++i) {
        if (i->second != r) continue;
    ...
    best_triplet_result = max(best_triplet_result,
                              real_hit / (data_model.data_test_true.size() +
                                          data_model.data_test_false.size()));
    ...
    for (auto i = data_model.data_test_true.begin();
      ...
      if (task_type == LinkPredictionRelation || part == 2) {
        for (auto j = 0; j != data_model.set_relation.size(); ++j) {
          ...
          if (score_i >= prob_triplets(t)) continue;

          ++rmean;

          if (data_model.check_data_all.find(t) ==
              data_model.check_data_all.end())
            ++frmean;
        }
      } else {
...
#pragma omp critical
      {
        if (frmean < hit_rank) ++arr_mean[data_model.relation_type[i->second]];
        ...
        if (rmean < hit_rank) ++hits;
        if (frmean < hit_rank) ++fhits;
      }
    }
  ...
  void test_link_prediction_head_tail(int hit_rank = 10, const int part = 0) {
    double mean = 0;
    double hits = 0;
    double fmean = 0;
    double fhits = 0;
...
#pragma omp parallel for
    for (auto i = data_model.data_test_true.begin();
      ...
      for (auto j = 0; j != data_model.set_entity.size(); ++j) {
        ...
        if (score_i < prob_triplets(th)) {
          ++rmeanh;

          if (data_model.check_data_all.find(th) ==
              data_model.check_data_all.end()) {
            ++frmeanh;
            // if (frmean > hit_rank)
            //	break;
        ...
        if (score_i < prob_triplets(tt)) {
          ++rmeant;

          if (data_model.check_data_all.find(tt) ==
              data_model.check_data_all.end()) {
            ++frmeant;
            // if (frmean > hit_rank)
            //	break;
...
#pragma omp critical
      {
        // for head
        if (frmeanh < hit_rank) ++arr_mean[data_model.relation_type[i->second]];
        ...
        if (rmeanh < hit_rank) ++hits;
        if (frmeanh < hit_rank) ++fhits;

        // for tail
        if (frmeant < hit_rank) ++arr_mean[data_model.relation_type[i->second]];
        ...
        if (rmeant < hit_rank) ++hits;
        if (frmeant < hit_rank) ++fhits;
      }
    }
    ...
    best_link_mean = min(best_link_mean, mean / total / 2);
    best_link_hitatten = max(best_link_hitatten, hits / total / 2);
    best_link_fmean = min(best_link_fmean, fmean / total / 2);
    best_link_fhitatten = max(best_link_fhitatten, fhits / total / 2);

    std::cout << "Raw.BestMEANS = " << best_link_mean << endl;
    std::cout << "Raw.BestMRR = " << rmrr / total / 2 << endl;
    std::cout << "Raw.BestHITS = " << best_link_hitatten << endl;
    logging.record() << "Raw.BestMEANS = " << best_link_mean;
    logging.record() << "Raw.BestMRR = " << rmrr / total / 2;
    logging.record() << "Raw.BestHITS = " << best_link_hitatten;

    std::cout << "Filter.BestMEANS = " << best_link_fmean << endl;
    std::cout << "Filter.BestMRR= " << fmrr / total / 2 << endl;
    std::cout << "Filter.BestHITS = " << best_link_fhitatten << endl;
    logging.record() << "Filter.BestMEANS = " << best_link_fmean;
    logging.record() << "Filter.BestMRR= " << fmrr / total / 2;
    logging.record() << "Filter.BestHITS = " << best_link_fhitatten;
...
#pragma omp critical
  ...
  virtual void draw(const string& filename, const int radius, const int id_head,
                    const int id_relation) {
    return;
  }

  virtual void report(const string& filename) const { return; }
 ...
 public:
  int count_entity() const { return data_model.set_entity.size(); }

  int count_relation() const { return data_model.set_relation.size(); }

  const DataModel& get_data_model() const { return data_model; }

 public:
  virtual void save(const string& filename) { cout << "BAD"; }

  virtual void load(const string& filename) { cout << "BAD"; }

  virtual vec entity_representation(int entity_id) const { cout << "BAD"; }

  virtual vec relation_representation(int relation_id) const { cout << "BAD"; }
};
