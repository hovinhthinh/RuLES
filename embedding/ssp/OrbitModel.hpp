...
class OrbitModel : public Model {
 ...
 public:
  OrbitModel(const Dataset& dataset, const TaskType& task_type,
 ...
 public:
  virtual void draw(const string& filename, const int radius, int id_head,
    ...
    for (auto i = data_model.rel_tails.at(id_relation).at(id_head).begin();
      ...
      record(50 +
                 radius * (3.0 + embedding_entity[*i][0] -
    ...
    for (auto i = 0;
         i < data_model.rel_tails.at(id_relation).at(id_head).size(); ++i) {
      heap_entity.pop();
    }
...
class OrbitE : public OrbitModel {
 public:
  OrbitE(const Dataset& dataset, const TaskType& task_type,
         const string& logging_base_path, int dim, double alpha,
         double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor_f = -sign(
        as_scalar(sum(abs(head_f + relation_f - tail_f)) - orbit * orbit));
...
class OrbitE2 : public OrbitModel {
 public:
  OrbitE2(const Dataset& dataset, const TaskType& task_type,
          const string& logging_base_path, int dim, double alpha,
          double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    double score = fabs(
        as_scalar((head + relation - tail).t() * (head + relation - tail)) -
        orbit * orbit);
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor = -sign(
        as_scalar((head + relation - tail).t() * (head + relation - tail)) -
        orbit * orbit);
    double factor_f = -sign(as_scalar((head_f + relation_f - tail_f).t() *
                                      (head_f + relation_f - tail_f)) -
                            orbit * orbit);
...
class OrbitHyper : public OrbitModel {
 protected:
  vector<vec> embedding_gap;

 public:
  OrbitHyper(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
 ...
 public:
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor = -sign(sum(abs(head - relation * head.t() * relation + gap -
                                  tail + relation * tail.t() * relation)) -
                          orbit * orbit);
    double factor_f =
        -sign(sum(abs(head_f - relation_f * head_f.t() * relation_f + gap_f -
                      tail_f + relation * tail_f.t() * relation)) -
              orbit * orbit);

    vec factor_vec = sign(head - relation * head.t() * relation + gap - tail +
                          relation * tail.t() * relation);

    head +=
        alpha * factor * (eye(dim, dim) - relation * relation.t()) * factor_vec;
    tail -=
        alpha * factor * (eye(dim, dim) - relation * relation.t()) * factor_vec;
    relation -= alpha * factor *
                ((eye(dim, dim) * as_scalar((head - tail).t() * relation) +
                  relation * (head - tail).t())) *
                factor_vec;
    gap += alpha * factor * factor_vec;

    vec factor_vec_f =
        sign(head_f - relation_f * head_f.t() * relation_f + gap_f - tail_f +
             relation_f * tail_f.t() * relation_f);
    head_f -= alpha * factor * (eye(dim, dim) - relation_f * relation_f.t()) *
              factor_vec_f;
    tail_f += alpha * factor * (eye(dim, dim) - relation_f * relation_f.t()) *
              factor_vec_f;
    relation_f +=
        alpha * factor *
        ((eye(dim, dim) * as_scalar((head_f - tail_f).t() * relation_f) +
          relation_f * (head_f - tail_f).t())) *
        factor_vec_f;
...
class OrbitE_ESS : public OrbitE {
 protected:
  double ESS_factor;

 public:
  OrbitE_ESS(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold, double ESS_factor)
      : OrbitE(dataset, task_type, logging_base_path, dim, alpha,
               training_threshold),
...
class OrbitE_BOX : public OrbitModel {
 public:
  OrbitE_BOX(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
...
//{
// protected:
//	vector<tuple<mat, boost::function<double(const double&)>,
//boost::function<double(const double&)>>>	weights;
//	const vector<int>
//network_architecture;
//
// public:
//	MultiLayerPerceptron(const vector<int>& network_architecture,
//		boost::function<double(const double&)> fn_active = [](const double
//x){return (exp(x)-exp(-x))/(exp(x)+exp(-x));},
//		boost::function<double(const double&)> fn_derv = [](const double
//x){return 1-x*x;})
...
//				randn(network_architecture[layer],
//network_architecture[layer+1]),
//				fn_active, fn_derv));
//		}
//	}
//
...
//		const double ndout,
//		boost::function<double(const double&)> fn_active = [](const double
//x){return 1.0/(1.0+exp(-x));},
//		boost::function<double(const double&)> fn_derv = [](const double
//x){return x*(1-x);})
//	{
//		weights.push_back(make_tuple(randn(ndin, ndout), fn_active,
//fn_derv));
//	}
//
// public:
...
//			for_each(dout.begin(), dout.end(), [&](double& elem){elem =
//get<1>(layer)(elem);});
//		}
//
//		return dout[0];
...
//
// public:
//	virtual void train_once(vec& head, vec& tail, const vec& dout, double
//alpha)
//	{
//		vector<vec>	hiddens;
...
//			for_each(hidden_out.begin(), hidden_out.end(), [&](double&
//elem){elem = get<1>(layer)(elem);});
...
//			{
//				derv[dim_derv] *=
//get<2>(weights[layer-1])(hiddens[layer][dim_derv]);
//			}
//
//			get<0>(weights[layer-1]) -= alpha * hiddens[layer-1] *
//derv.t();
//			derv = get<0>(weights[layer-1]) * derv;
//		}
//
//		head -= alpha * derv.rows(0, head.size()-1);
//		tail -= alpha * derv.rows(head.size(), head.size() +
//tail.size()-1);
//	}
//};
//
...
//		{
//			mlp.push_back(MultiLayerPerceptron(na, fn_active,
//fn_derv));
//		}
//
...
//		double& orbit = embedding_orbit[triplet.second];
//
//		double score = fabs(mlp[triplet.second].infer(join_cols(head,
//tail)) - orbit*orbit);
//		return - score;
//	}
//
...
//		data_model.sample_false_triplet(triplet, triplet_f);
//
//		if (prob_triplets(triplet) - prob_triplets(triplet_f) >
//training_threshold)
//			return;
//
//		vec& head_f = embedding_entity[triplet_f.first.first];
...
//
//		double factor =
//			- sign(mlp[triplet.second].infer(join_cols(head, tail)) -
//orbit*orbit);
//		double factor_f =
//			- sign(mlp[triplet_f.second].infer(join_cols(head_f,
//tail_f)) - orbit*orbit);
//
//		vec dout(1);
//		dout << orbit*orbit;
...
class OrbitE_H : public OrbitModel {
 protected:
  vector<vec> embedding_weights;

 public:
  OrbitE_H(const Dataset& dataset, const TaskType& task_type,
           const string& logging_base_path, int dim, double alpha,
           double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
    ...
    for (auto i = embedding_weights.begin(); i != embedding_weights.end();
         ++i) {
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor_f =
        -sign(as_scalar((head_f + relation_f).t() * tail_f) - orbit);
...
class OrbitE_HD : public OrbitModel {
 protected:
  vector<vec> embedding_weights;

 public:
  OrbitE_HD(const Dataset& dataset, const TaskType& task_type,
            const string& logging_base_path, int dim, double alpha,
            double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    double score =
        fabs(as_scalar((head + relation).t() * (tail + weight)) - orbit);
    return -score;
  }
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor =
        -sign(as_scalar((head + relation).t() * (tail + weight)) - orbit);
    double factor_f = -sign(
        as_scalar((head_f + relation_f).t() * (tail_f + weight_f)) - orbit);
...
class OrbitE_HDA : public OrbitModel {
 protected:
  vector<vec> embedding_weights;

 public:
  OrbitE_HDA(const Dataset& dataset, const TaskType& task_type,
             const string& logging_base_path, int dim, double alpha,
             double training_threshold)
      : OrbitModel(dataset, task_type, logging_base_path, dim, alpha,
                   training_threshold) {
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    double score =
        fabs(as_scalar(abs(head + relation).t() * abs(tail + weight)) - orbit);
    return -score;
  }
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor =
        -sign(as_scalar(abs(head + relation).t() * abs(tail + weight)) - orbit);
    double factor_f =
        -sign(as_scalar(abs(head_f + relation_f).t() * abs(tail_f + weight_f)) -
              orbit);
    ...
    head_f -=
        alpha * factor_f * sign(head_f + relation_f) % abs(tail_f + weight_f);
    relation_f -=
        alpha * factor_f * sign(head_f + relation_f) % abs(tail_f + weight_f);
    tail_f -=
        alpha * factor_f * sign(tail_f + weight_f) % abs(head_f + relation_f);
    weight_f -=
        alpha * factor_f * sign(tail_f + weight_f) % abs(head_f + relation_f);
...
class OrbitE_KS : public OrbitModel {
 protected:
  vector<vec> embedding_weights;
 ...
 public:
  OrbitE_KS(const Dataset& dataset, const TaskType& task_type,
            const string& logging_base_path, int dim, double alpha,
            double training_threshold,
            boost::function<double(const vec& a, const vec& b)> kernel,
            boost::function<vec(const vec& a, const vec& b)> derv_a,
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    double score = fabs(kernel(head, head) + kernel(tail, tail) +
                        kernel(relation, relation) +
                        2 * (kernel(head, relation) - kernel(head, tail) -
                             kernel(relation, tail)) -
                        orbit);
    return -score;
  }
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor = -sign(kernel(head, head) + kernel(tail, tail) +
                          kernel(relation, relation) +
                          2 * (kernel(head, relation) - kernel(head, tail) -
                               kernel(relation, tail)) -
                          orbit);
    double factor_f =
        -sign(kernel(head_f, head_f) + kernel(tail_f, tail_f) +
              kernel(relation_f, relation_f) +
              2 * (kernel(head_f, relation_f) - kernel(head_f, tail_f) -
                   kernel(relation_f, tail_f)) -
              orbit);

    head +=
        alpha * factor * (derv_a(head, head) + derv_b(head, head) +
                          2 * derv_a(head, relation) - 2 * derv_a(head, tail));
    relation += alpha * factor *
                (derv_a(relation, relation) + derv_b(relation, relation) +
                 2 * derv_b(head, relation) - 2 * derv_a(relation, tail));
    tail +=
        alpha * factor * (derv_a(tail, tail) + derv_b(tail, tail) -
                          2 * derv_b(head, tail) - 2 * derv_b(relation, tail));

    head_f -= alpha * factor_f *
              (derv_a(head_f, head_f) + derv_b(head_f, head_f) +
               2 * derv_a(head_f, relation_f) - 2 * derv_a(head_f, tail_f));
    relation_f -=
        alpha * factor_f *
        (derv_a(relation_f, relation_f) + derv_b(relation_f, relation_f) +
         2 * derv_b(head_f, relation_f) - 2 * derv_a(relation_f, tail_f));
    tail_f -= alpha * factor_f *
              (derv_a(tail_f, tail_f) + derv_b(tail_f, tail_f) -
               2 * derv_b(head_f, tail_f) - 2 * derv_b(relation_f, tail_f));
...
class OrbitE_KHDA : public OrbitModel {
 protected:
  vector<vec> embedding_weights;
  boost::function<double(const vec& a, const vec& b)> kernel;
  boost::function<vec(const vec& a, const vec& b)> derv_a;
 ...
 public:
  OrbitE_KHDA(const Dataset& dataset, const TaskType& task_type,
              const string& logging_base_path, int dim, double alpha,
              double training_threshold,
              boost::function<double(const vec& a, const vec& b)> kernel,
              boost::function<vec(const vec& a, const vec& b)> derv_a,
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    ...
    double score =
        fabs(kernel(head, tail) + kernel(relation, tail) +
             kernel(head, weight) + kernel(relation, weight) - orbit);
    return -score;
  }
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor =
        -sign(kernel(head, tail) + kernel(relation, tail) +
              kernel(head, weight) + kernel(relation, weight) - orbit);
    double factor_f =
        -sign(kernel(head_f, tail_f) + kernel(relation_f, tail_f) +
              kernel(head_f, weight_f) + kernel(relation_f, weight_f) - orbit);

    head += alpha * factor * (derv_a(head, tail) + derv_a(head, weight));
    relation +=
        alpha * factor * (derv_a(relation, tail) + derv_a(relation, weight));
    tail += alpha * factor * (derv_b(relation, tail) + derv_b(head, tail));
    weight +=
        alpha * factor * (derv_b(head, weight) + derv_b(relation, weight));

    head_f -=
        alpha * factor_f * (derv_a(head_f, tail_f) + derv_a(head_f, weight_f));
    relation_f -= alpha * factor_f *
                  (derv_a(relation_f, tail_f) + derv_a(relation_f, weight_f));
    tail_f -= alpha * factor_f *
              (derv_b(relation_f, tail_f) + derv_b(head_f, tail_f));
    weight_f -= alpha * factor_f *
                (derv_b(head_f, weight_f) + derv_b(relation_f, weight_f));
...
class OrbitE_KHDAN : public OrbitModel {
 protected:
  vector<vec> embedding_weights;
  boost::function<double(const vec& a, const vec& b)> kernel;
  boost::function<vec(const vec& a, const vec& b)> derv_a;
 ...
 public:
  OrbitE_KHDAN(const Dataset& dataset, const TaskType& task_type,
               const string& logging_base_path, int dim, double alpha,
               double training_threshold,
               boost::function<double(const vec& a, const vec& b)> kernel,
               boost::function<vec(const vec& a, const vec& b)> derv_a,
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    double factor_f =
        -sign(kernel(head_f + relation_f, tail_f + weight_f) - orbit);
    ...
    relation_f -=
        alpha * factor_f * derv_a(head_f + relation_f, tail_f + weight_f);
    tail_f -= alpha * factor_f * derv_b(head_f + relation_f, tail_f + weight_f);
    weight_f -=
        alpha * factor_f * derv_b(head_f + relation_f, tail_f + weight_f);
