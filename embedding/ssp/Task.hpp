...
class TopicRegressionTask {
 ...
 public:
  TopicRegressionTask(Model* model, const string type_file,
 ...
 public:
  ...
  void train(int epos, double alpha) {
    ...
    while (epos-- > 0) {
      ...
      for (auto i = topics.begin(); i != topics.end(); ++i) {
        ++pos;
        if (pos % 10 == 0) continue;

        const vec& input = input_sample[i - topics.begin()];
        int now_pos = 0;
        for (auto it = topics_training[i - topics.begin()].begin();
             it != topics_training[i - topics.begin()].end(); ++it) {
  ...
  void test() {
    ...
    for (auto i = topics.begin(); i != topics.end(); ++i) {
      ...
      for (auto it = lrc.begin(); it != lrc.end(); ++it) {
        ...
        scores.push_back(
            make_pair(score, topic_list_id_to_raw[it - lrc.begin()]));
      }
      ...
      for (auto iscore = scores.begin(); iscore != scores.end(); ++iscore) {
        if (i->find(iscore->second) == i->end()) continue;

        ++cnt_ft;
        hit += cnt_ft / double(iscore - scores.begin() + 1);

        if (cnt_ft == i->size()) break;
      }
    ...
    best = max(hits, best);
    cout << endl
         << "Entity Classification = " << hits << ", Best = " << best << endl;
    model->logging.record() << "Entity Classification MAP = " << hits;
    model->logging.record() << "Best = " << best;
  }
};

class TopicRegressionTaskZeroShot : public TopicRegressionTask {
 public:
  TopicRegressionTaskZeroShot(TransE* model, const string type_file,
                              const int evaluted_type = 50)
 ...
 public:
  void train(int epos, double alpha) {
    ...
    while (epos-- > 0) {
      ...
      for (auto i = topics.begin(); i != topics.end(); ++i) {
        ++pos;
        if (pos >= model->get_data_model().zeroshot_pointer) break;
...
// model->run(100);
// model->draw("D:\\OrbitE.philosopher.profession.big.ppm", 500,
// model->get_data_model().entity_name_to_id.at("philosopher"),
...
//				{
//					if (dm.rel_finder.find(make_pair(*a, *c)) !=
//dm.rel_finder.end() &&
//						dm.rel_finder.find(make_pair(*b,
//*c))->second == dm.rel_finder.find(make_pair(*a, *c))->second)
//					{
//						if (dm.rel_finder.find(make_pair(*a,
//*c))->second == i->first)
//							continue;
//						log.record();
//						log.record()<<dm.relation_id_to_name[i->first];
//						log.record()<<dm.relation_id_to_name[dm.rel_finder.find(make_pair(*a,
//*c))->second];
...
// model = new TransA(FB13, TripletClassification, report_path, 200, 0.00175,
// 3.2);
// model->run(8000);
// model->test();
// delete model;
