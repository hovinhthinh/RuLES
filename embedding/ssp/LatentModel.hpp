...
class LatentModel : public Model {
 protected:
 ...
 public:
  LatentModel(const Dataset& dataset, const TaskType& task_type,
              const string& logging_base_path, int topic)
    ...
    for (auto i = embedding_relation.begin(); i != embedding_relation.end();
         ++i) {
      *i = zeros(n_topic, 1);
    }
 ...
 public:
  void normalize_to_prob() {
    for (auto i = 0; i < n_topic; ++i) {
      double total_sum = 0;
      for (auto item = embedding_head.begin(); item != embedding_head.end();
           ++item) {
        total_sum += (*item)(i);
      }
    ...
    for (auto i = 0; i < n_topic; ++i) {
      double total_sum = 0;
      for (auto item = embedding_tail.begin(); item != embedding_tail.end();
           ++item) {
        total_sum += (*item)(i);
      }
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    return sum(embedding_head(triplet.second, triplet.first.first) %
               embedding_tail(triplet.second, triplet.first.second) %
               embedding_topic / embedding_relation[triplet.second]) +
           1e-100;
  }

  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    embedding_head_comp(triplet.second, triplet.first.first) +=
        embedding_head(triplet.second, triplet.first.first) % embedding_topic /
        prob_triplets(triplet);
    embedding_tail_comp(triplet.second, triplet.first.second) +=
        embedding_tail(triplet.second, triplet.first.second) % embedding_topic /
        prob_triplets(triplet);
    embedding_topic_comp += embedding_topic / prob_triplets(triplet);
  }

  virtual void train(bool last_time = false) {
    for (auto i = embedding_head_comp.begin(); i != embedding_head_comp.end();
         ++i) {
      *i = zeros(n_topic, 1);
    }

    for (auto i = embedding_tail_comp.begin(); i != embedding_tail_comp.end();
         ++i) {
      *i = zeros(n_topic, 1);
    }
...
class PropergationModel : public Model {
 protected:
 ...
 public:
  PropergationModel(const Dataset& dataset, const TaskType& task_type,
    ...
    for_each(relation_in.begin(), relation_in.end(), [&](vec& elem) {
      elem = (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim);
    });

    relation_out.resize(count_relation());
    for_each(relation_out.begin(), relation_out.end(), [&](vec& elem) {
      elem = ones(dim);
      (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim);
    });

    entity.resize(count_entity());
    for_each(entity.begin(), entity.end(), [&](vec& elem) {
      elem = (2 * randu(dim, 1) - 1) * sqrt(6.0 / dim);
    });
  }
  ...
  virtual double prob_triplets(const pair<pair<int, int>, int>& triplet) {
    double score =
        sum(abs(entity[triplet.first.first] % relation_in[triplet.second] -
  ...
  virtual void train_triplet(const pair<pair<int, int>, int>& triplet) {
    ...
    vec factor_vec =
        -sign(entity[triplet.first.first] % relation_in[triplet.second] -
              entity[triplet.first.second] % relation_out[triplet.second]);
    ...
    factor_vec =
        -sign(entity[triplet_f.first.first] % relation_in[triplet_f.second] -
    ...
    if (norm_L2(entity[triplet_f.first.second]) > 1)
      entity[triplet_f.first.second] =
          normalise(entity[triplet_f.first.second]);
...
class FactorEKL : public Model {
 ...
 public:
  FactorEKL(const Dataset& dataset, const TaskType& task_type,
            const string& logging_base_path, int dim, double alpha,
            double training_threshold, double smoothing)
      : Model(dataset, task_type, logging_base_path),
        dim(dim),
        alpha(alpha),
        margin(training_threshold),
        smoothing(smoothing) {
 ...
 public:
  ...
  virtual void train_derv(const pair<pair<int, int>, int>& triplet,
...
class FactorE : public FactorEKL {
 protected:
  const double sigma;

 public:
  FactorE(const Dataset& dataset, const TaskType& task_type,
          const string& logging_base_path, int dim, double alpha,
          double training_threshold, double sigma)
      : FactorEKL(dataset, task_type, logging_base_path, dim, alpha,
 ...
 public:
  virtual void train_triplet(
    ...
    if (prob_triplets(triplet) - prob_triplets(triplet_f) > margin) return;
...
class MFactorE : public Model {
 ...
 public:
  MFactorE(const Dataset& dataset, const TaskType& task_type,
           const string& logging_base_path, int dim, double alpha,
           double training_threshold, double sigma, int n_factor)
      : Model(dataset, task_type, logging_base_path),
        dim(dim),
        alpha(alpha),
        margin(training_threshold),
    ...
    for (auto i = 0; i < n_factor; ++i) {
      factors.push_back(
          new SFactorE(dim, count_entity(), count_relation(), sigma));
    }
  }
 ...
 public:
  virtual void train_triplet(
    ...
    for (auto i = 0; i < n_factor; ++i) {
      factors[i]->train(triplet,
                        n_factor * alpha * relation_space[triplet.second][i]);
      factors[i]->train(triplet_f,
                        -n_factor * alpha * relation_space[triplet.second][i]);
    }
 ...
 public:
  virtual vec entity_representation(int entity_id) const {
    ...
    for (auto i = 0; i < n_factor; ++i) {
      rep_vec =
          join_cols(rep_vec, factors[i]->entity_representation(entity_id));
    }
...
class MFactorSemantics : public MFactorE {
 protected:
 ...
 public:
  MFactorSemantics(const Dataset& dataset, const TaskType& task_type,
                   const string& logging_base_path,
                   const string& semantic_file_raw, int dim, double alpha,
                   double training_threshold, double sigma, int n_factor)
      : MFactorE(dataset, task_type, logging_base_path, dim, alpha,
                 training_threshold, sigma, n_factor) {
    ...
    while (!fin.eof()) {
      ...
      documents[data_model.entity_name_to_id.find(entity_name)->second] =
          entity_description;
