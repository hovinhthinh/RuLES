...
class Experiment(object):
    def __init__(self):
        ...
        self.parser.add_argument('--workspace', type=str, help='Folder to input/output', default=None)
        ...
        self.parser.add_argument('--output', type=str, default='embedding')
        self.neval = -1
    ...
    def run(self):
        ...
        if self.args.mode == 'rank':
            ...
            raise ValueError('Deprecated experiment mode (%s)' % self.args.mode)
    ...
    def read_data(self):
        data = {}
        with open(self.args.workspace + '/meta.txt', 'r') as fin:
            nm = fin.readline().strip().split('\t')
            n, m = int(nm[0]), int(nm[1])
            data['entities'] = []
            for i in range(n):
                data['entities'].append(fin.readline().strip())
            data['relations'] = []
            for i in range(m):
                data['relations'].append(fin.readline().strip())
        with open(self.args.workspace + '/train.txt', 'r') as fin:
            data['train_subs'] = []
            for line in fin:
                nm = line.strip().split('\t')
                data['train_subs'].append((int(nm[0]), int(nm[2]), int(nm[1])))
        with open(self.args.workspace + '/valid.txt', 'r') as fin:
            data['valid_subs'] = []
            for line in fin:
                nm = line.strip().split('\t')
                data['valid_subs'].append((int(nm[0]), int(nm[2]), int(nm[1])))
        with open(self.args.workspace + '/test.txt', 'r') as fin:
            data['test_subs'] = []
            for line in fin:
                nm = line.strip().split('\t')
                data['test_subs'].append((int(nm[0]), int(nm[2]), int(nm[1])))
        return data

    def write_model(self, trn):
        log.info('Save model...')
        model = trn.model
        # save embedding
        with open(self.args.workspace + '/' + self.args.output, 'wb') as fout:
            fout.write(struct.pack('>d', len(model.E[0])))
            for m in (model.E, model.R):
                for e in m:
                    for val in e:
                        fout.write(struct.pack('>d', val))

    def ranking_callback(self, trn, with_eval=False, with_test=False):
        # print basic info
        ...
        if (trn.epoch % self.args.test_all == 0) or with_eval:
            if with_test:
                pos_v, fpos_v = self.ev_test.positions(trn.model)
                fmrr_valid = ranking_scores(pos_v, fpos_v, trn.epoch, 'TEST')
            else:
            ...
            if fmrr_valid > self.best_valid_score:
                ...
                if not with_test:
                    self.write_model(trn)
            else:
                return False

        return True

    def lp_callback(self, m, with_eval=False, with_test=False):
        # print basic info
